#!/usr/bin/env bash
#
# Run extract_icon_json.sh against all first-party macOS apps,
# capture fidelity scores, and compute the average.
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
EXTRACT="$SCRIPT_DIR/extract_icon_json.sh"
...
for app in "${APPS[@]}"; do
  ...
  # Run the extraction script, capture output
  output=$("$EXTRACT" "$app" 2>&1) || {
...
if [ ${#SCORES[@]} -gt 0 ]; then
  ...
  for app in "${APPS[@]}"; do
    ...
    if [ -d "./${app_name}.icon" ] && [ $idx -lt ${#SCORES[@]} ]; then
      printf "  %-30s %s/100\n" "$app_name" "${SCORES[$idx]}"
      idx=$((idx + 1))
    fi
  done
